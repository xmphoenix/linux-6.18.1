...
### 1.1 用户态工具 (`tools/perf/`)

#### 1.1.1 主入口与构建系统

| 文件 | 说明 |
|------|------|
| `tools/perf/perf.c` | **主入口**，所有子命令的调度中心 |
| `tools/perf/Makefile` | 顶层 Makefile 包装器，调用 Makefile.perf 并行编译 |
| `tools/perf/Makefile.perf` | 核心构建系统，支持交叉编译、特性检测 |
| `tools/perf/Makefile.config` | 编译配置选项检测 |
| `tools/perf/builtin.h` | 所有 builtin 命令的声明 |
| `tools/perf/perf.h` | perf 全局头文件 |
| `tools/perf/perf-sys.h` | 系统调用封装 |

#### 1.1.2 Builtin 子命令 (31 个)

**性能剖析类：**

| 文件 | 命令 | 功能 |
|------|------|------|
| `builtin-record.c` | `perf record` | 采样记录，将 CPU/进程的性能数据写入 `perf.data` |
| `builtin-report.c` | `perf report` | 分析 `perf.data`，以直方图形式展示热点函数 |
| `builtin-annotate.c` | `perf annotate` | 将采样数据标注到源码/汇编级别 |
| `builtin-top.c` | `perf top` | **实时**持续显示系统热点（类似 `top`） |
| `builtin-stat.c` | `perf stat` | 精确的性能计数器统计概览 |

**追踪调试类：**

| 文件 | 命令 | 功能 |
|------|------|------|
| `builtin-trace.c` | `perf trace` | 持续系统调用追踪（类似 `strace`，依赖 libtraceevent） |
| `builtin-probe.c` | `perf probe` | 设置动态探测点（kprobe/uprobe），依赖 libelf |
| `builtin-ftrace.c` | `perf ftrace` | 内核 ftrace 子系统的前端 |
| `builtin-script.c` | `perf script` | 用脚本解析 trace event 数据 |

**专项分析类：**

| 文件 | 命令 | 功能 |
|------|------|------|
| `builtin-c2c.c` | `perf c2c` | Cache-to-Cache 通信分析（伪共享检测） |
| `builtin-mem.c` | `perf mem` | 内存访问模式分析 |
| `builtin-lock.c` | `perf lock` | 锁竞争分析（依赖 libtraceevent） |
| `builtin-sched.c` | `perf sched` | 调度器追踪分析（依赖 libtraceevent） |
| `builtin-kmem.c` | `perf kmem` | 内核内存分配分析（依赖 libtraceevent） |
| `builtin-kvm.c` | `perf kvm` | KVM 虚拟化追踪 |
| `builtin-kwork.c` | `perf kwork` | 内核 work 分析（依赖 libtraceevent） |
| `builtin-timechart.c` | `perf timechart` | 生成 SVG 时间图（依赖 libtraceevent） |
| `builtin-bench.c` | `perf bench` | 内核性能基准测试 |
| `builtin-diff.c` | `perf diff` | 比较两个 `perf.data` 文件的差异 |

**工具类：**

| 文件 | 命令 | 功能 |
|------|------|------|
| `builtin-list.c` | `perf list` | 列出所有可用事件类型 |
| `builtin-evlist.c` | `perf evlist` | 列出 `perf.data` 中的事件 |
| `builtin-buildid-cache.c` | `perf buildid-cache` | 管理 build-id 缓存 |
| `builtin-buildid-list.c` | `perf buildid-list` | 列出 `perf.data` 中的 build ID |
| `builtin-config.c` | `perf config` | 管理 perf 配置 |
| `builtin-daemon.c` | `perf daemon` | 守护进程模式 |
| `builtin-data.c` | `perf data` | 数据文件操作 |
| `builtin-inject.c` | `perf inject` | 向 `perf.data` 注入事件 |
| `builtin-kallsyms.c` | `perf kallsyms` | 符号查找 |
| `builtin-check.c` | `perf check` | 系统环境检查 |
| `builtin-version.c` | `perf version` | 版本信息 |

#### 1.1.3 核心工具库 (`tools/perf/util/`)
...
| 文件 | 功能 |
|------|------|
| `evlist.c / evsel.c` | 事件列表/事件选择器管理 |
| `session.c` | perf 会话管理（打开/关闭 perf.data） |
| `machine.c` | 机器级别抽象（内核/用户空间信息） |
| `map.c / maps.c` | 内存映射管理 |
| `dso.c` | 共享对象（DSO）管理 |
| `symbol.c` | 符号表解析 |
| `thread.c` | 线程抽象 |
...
| 文件 | 功能 |
|------|------|
| `parse-events.l / .y` | 事件表达式解析器（flex/bison 语法） |
| `pmu.l / .y` | PMU 事件解析器 |
| `expr.l / .y` | 通用表达式解析器 |
| `pmu.c` | PMU 检测与管理 |
| `trace-event.c` | trace event 处理 |
| `tracepoint.c` | tracepoint 处理 |
...
|------|------|
| `debuginfo.c` | 调试信息处理 |
| `dwarf-aux.c` | DWARF 辅助工具 |
| `annotate.c` | 源码标注 |
| `callchain.c` | 调用链分析 |
| `unwind-libdw.c` | libdw 栈回溯 |
| `unwind-libunwind.c` | libunwind 栈回溯 |

**BPF 支持：**

| 文件 | 功能 |
|------|------|
| `bpf_counter.c` | BPF 计数器 |
| `bpf_filter.l / .y` | BPF 过滤表达式 |
| `bpf-event.c` | BPF 事件处理 |
| `bpf_skel/` | BPF skeleton 模板 |

**AUX trace 支持（架构特定）：**

| 文件 | 功能 |
|------|------|
| `auxtrace.c` | AUX trace 框架 |
| `arm-spe.c` | ARM SPE (Statistical Profiling Extension) 解码 |
| `cs-etm.c` | ARM CoreSight ETM 解码 |
| `intel-pt.c` | Intel Processor Trace 解码 |
| `intel-bts.c` | Intel Branch Trace Store |

#### 1.1.4 ARM64 架构特定代码 (`tools/perf/arch/arm64/`)

| 文件 | 功能 |
|------|------|
| `util/arm-spe.c` | ARM SPE 支持 |
| `util/header.c` | perf.data 头部处理 |
| `util/hisi-ptt.c` | 华为海思 PTT 设备支持 |
| `util/kvm-stat.c` | KVM 统计 |
| `util/machine.c` | ARM64 机器初始化 |
| `util/mem-events.c` | 内存事件 |
| `util/perf_regs.c` | 寄存器定义 |
| `util/pmu.c` | PMU 设置 |
| `util/tsc.c` | 时钟管理 |
| `util/unwind-libdw.c` | libdw 栈回溯 |
| `util/unwind-libunwind.c` | libunwind 栈回溯 |
| `annotate/` | ARM64 指令标注 |
...
#### 1.2.1 核心子系统 (`kernel/events/`)

| 文件 | 功能 |
|------|------|
| `core.c` | **perf event 核心**，~14000 行，包含 `sys_perf_event_open` 系统调用、事件调度、采样、计数 |
| `ring_buffer.c` | 环形缓冲区实现，内核→用户态数据传递 |
| `callchain.c` | 调用链遍历 |
| `hw_breakpoint.c` | 硬件断点支持 |
| `uprobes.c` | 用户空间探测点 |
| `internal.h` | 内部数据结构定义 |
...
| 文件 | 功能 |
|------|------|
| `arch/arm64/kernel/perf_callchain.c` | ARM64 调用链遍历 |
| `arch/arm64/kernel/perf_regs.c` | ARM64 寄存器访问 |
| `arch/arm64/kernel/hw_breakpoint.c` | ARM64 硬件断点 |
| `arch/arm64/kernel/watchdog_hld.c` | PMU NMI 看门狗 |
...
#### 1.2.3 ARM64 PMU 驱动 (`drivers/perf/`)

| 文件 | 功能 |
|------|------|
| `arm_pmu.c` | ARM PMU 通用框架 |
| `arm_pmuv3.c` | **ARMv8 PMUv3** 驱动（ARM64 主力 PMU） |
| `arm_brbe.c` | ARM Branch Record Buffer Extension |
| `arm_spe_pmu.c` | ARM Statistical Profiling Extension |
| `arm_pmu_platform.c` | 平台设备绑定 |
| `arm_pmu_acpi.c` | ACPI 绑定 |
| `arm_dsu_pmu.c` | DynamIQ Shared Unit PMU |
| `arm_dmc620_pmu.c` | DMC-620 内存控制器 PMU |
| `arm_smmuv3_pmu.c` | SMMUv3 PMU |
| `arm-cmn.c` | CMN (Coherent Mesh Network) PMU |
| `arm-ccn.c` | CCN (Cache Coherent Network) PMU |
| `arm-cci.c` | CCI (Cache Coherent Interconnect) PMU |
| `arm-ni.c` | NI (Network-on-chip Interconnect) PMU |

#### 1.2.4 公共头文件

| 文件 | 功能 |
|------|------|
| `include/uapi/linux/perf_event.h` | **用户空间 ABI**：`perf_event_attr`、事件类型枚举、采样格式 |
| `include/linux/perf_event.h` | **内核内部**：`struct pmu`、`struct perf_event`、回调接口 |
| `include/linux/perf/arm_pmu.h` | ARM PMU 框架头文件 |
| `include/linux/perf/arm_pmuv3.h` | ARMv8 PMUv3 寄存器定义 |
| `include/linux/hw_breakpoint.h` | 硬件断点定义 |
| `include/linux/perf_regs.h` | 寄存器定义（栈回溯用） |
| `include/uapi/linux/bpf_perf_event.h` | BPF perf event 集成 |

---

## 2. Perf 依赖的内核选项与用户态依赖
...
> `PERF_EVENTS` 是所有 perf 功能的总开关。ARM64 架构通过 `select HAVE_PERF_EVENTS` 自动满足依赖。
...
| 配置选项 | 说明 |
|---------|------|
| `CONFIG_PERF_EVENTS=y` | **必须**，perf 核心 |
| `CONFIG_HW_PERF_EVENTS=y` | 硬件性能计数器（ARM64 默认开启） |
| `CONFIG_TRACEPOINTS=y` | tracepoint 事件支持 |
| `CONFIG_FTRACE=y` | ftrace 框架，`perf ftrace` 需要 |
| `CONFIG_KPROBES=y` | kprobe 动态探测，`perf probe` 需要 |
| `CONFIG_UPROBES=y` | uprobe 用户空间探测 |
| `CONFIG_KALLSYMS=y` | 内核符号表 |
| `CONFIG_KALLSYMS_ALL=y` | 所有符号（不仅是函数） |
| `CONFIG_DEBUG_INFO=y` | DWARF 调试信息（源码级标注） |
| `CONFIG_DEBUG_INFO_DWARF5=y` | DWARF5 格式（推荐） |
| `CONFIG_FRAME_POINTER=y` | 帧指针（更精确的调用链） |
| `CONFIG_STACKTRACE=y` | 栈回溯支持 |
| `CONFIG_BPF_SYSCALL=y` | BPF 系统调用支持 |
| `CONFIG_CGROUP_PERF=y` | cgroup perf 支持 |
| `CONFIG_PROFILING=y` | 通用 profiling 基础设施 |
| `CONFIG_GUEST_PERF_EVENTS=y` | KVM 客户机 perf |
...
| 参数 | 说明 |
|------|------|
| `/proc/sys/kernel/perf_event_paranoid` | 访问控制级别：`-1`=无限制, `0`=允许所有用户, `1`=仅自身进程, `2`=仅软件事件（默认） |
| `/proc/sys/kernel/perf_event_max_sample_rate` | 最大采样频率 |
| `/proc/sys/kernel/perf_event_mlock_kb` | perf mmap 缓冲区锁定内存上限 |
| `/proc/sys/kernel/perf_event_max_stack` | 调用链最大深度 |
| `/proc/sys/kernel/kptr_restrict` | 内核指针暴露级别（设为 `0` 解除符号地址限制） |
...
### 3.2 核心系统调用：`perf_event_open()`

perf 的一切始于一个系统调用（定义于 `kernel/events/core.c:13387`）：
...
`perf_event_attr.type` 决定事件来源（定义于 `include/uapi/linux/perf_event.h`）：
...
- `perf_event_attr.sample_period = 0`
- 内核持续累计硬件计数器值
- 用户态通过 `read(fd)` 读取精确计数
- 开销极低，接近零

**数据流**：
```
CPU PMU 计数器 → perf_event.count → read(fd) → perf stat 展示
```

#### 3.4.2 采样模式 (Sampling)

```
perf record -e cycles -c 100000 ./program
```

- `perf_event_attr.sample_period > 0`（或 `freq=1` + `sample_freq`）
- 每 N 个事件触发一次 PMU 溢出中断（PMI）
- 中断处理程序记录当前 IP、调用链、时间戳等到**环形缓冲区**
- 用户态通过 `mmap()` 读取环形缓冲区
...
### 3.5 PMU 驱动模型 (`struct pmu`)

每种事件源由一个 `struct pmu` 实例驱动（定义于 `include/linux/perf_event.h:329`）：
...
| `PMSELR_EL0` | 计数器选择 |
| `PMCCNTR_EL0` | 周期计数器 |
| `PMEVCNTRn_EL0` | 事件计数器 (n=0..30) |
| `PMEVTYPERn_EL0` | 事件类型寄存器 |
| `PMINTENSET_EL1` | 中断使能 |
| `PMOVSCLR_EL0` | 溢出状态清除 |
...
**关键流程**：
1. 内核在 PMI 中断中调用 `perf_output_begin()` → `perf_output_put()` → `perf_output_end()`
2. 更新 `data_head`（写指针）
3. 用户态轮询 `data_head`，读取新数据
4. 用户态更新 `data_tail`（读指针）通知内核已消费

### 3.7 事件调度算法

当 PMU 硬件计数器数量有限时（ARM64 PMUv3 典型为 6 个），内核需要调度事件：
...
main() → cmd_record()
  ...
  → disable_counters()           # ioctl(PERF_EVENT_IOC_DISABLE)
```
...
```bash
# Ubuntu/Debian 安装 aarch64 交叉编译依赖
...
sudo apt-get install libbabeltrace-dev:arm64
```

> **注意**：如果无法安装 arm64 的库包，可以选择禁用对应功能来编译最小化版本。
...
> **注意**：最小化编译将缺失大量功能（符号解析、trace、BPF 等），仅适合基础计数。
...
```bash
make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- \
    O=/tmp/perf-arm64-build
```

#### 4.2.5 静态编译（适合目标系统无共享库）
...
# 或者直接复制编译产物
scp tools/perf/perf root@target:/usr/local/bin/
```

### 4.4 编译特性检测

编译过程中 Makefile.config 会自动检测可用的库和特性，输出类似：
...
使用 `V=1` 查看详细编译命令，`VF=1` 查看特性检测细节：

```bash
make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- VF=1
```
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| `fatal error: elf.h: No such file` | 缺少 libelf 开发包 | 安装 `libelf-dev:arm64` 或 `NO_LIBELF=1` |
| `cannot find -lunwind` | 缺少 libunwind 交叉编译库 | 安装或指定 `LIBUNWIND_DIR=` |
| `No rule to make target 'libtraceevent'` | libtraceevent 未安装 | `NO_LIBTRACEEVENT=1` |
| `multiple definition of ...` | 工具链版本兼容 | 添加 `EXTRA_CFLAGS=-fcommon` |
| `flex/bison not found` | 缺少解析器生成器 | `apt install flex bison` |
...
echo  0 > /proc/sys/kernel/kptr_restrict          # 暴露内核符号地址
```
...
ARMv8 Performance Monitors Extension（PMUv3）定义于 ARM Architecture Reference Manual D13 章节，是 ARM64 平台性能分析的硬件基础。自 ARMv8.1 起持续增强，目前演进到 PMUv3p9（ARMv8.9/ARMv9.4）。
...
[15:11] N     只读。实现的通用事件计数器数量（典型值：4-8）
```

> **Linux 内核操作**：`armv8pmu_pmcr_write()` 写入时用 `ARMV8_PMU_PMCR_MASK` 屏蔽保留位，并在写入前执行 `isb()` 指令序列化。

#### 5.2.2 PMEVTYPERn_EL0 — 事件类型寄存器
...
[63:61]   TC         阈值比较类型（v8.8+）
```

#### 5.2.3 PMUv3 事件编号空间

PMUv3 定义了超过 100 个架构事件（`include/linux/perf/arm_pmuv3.h`）：

| 事件范围 | 类别 | 示例 |
|---------|------|------|
| `0x0000-0x003F` | 架构通用事件 | `0x0011`=CPU_CYCLES, `0x0008`=INST_RETIRED |
| `0x0040-0x00FF` | 推荐微架构事件 | `0x0040`=L1D_CACHE_RD, `0x0070`=LD_SPEC |
| `0x4000-0x403F` | 扩展架构事件 | `0x4000`=SPE_SAMPLE_POP, `0x4004`=AMU_CNT_CYCLES |
| `0xC000-0xFFFF` | 厂商自定义事件 | A53: `0xC2`=PREF_LINEFILL |
...
PMCCNTR_EL0（周期计数器，始终 64 位）:
  ...
  当 PMCR.LC=0 时仅低 32 位有效
```

### 5.5 Bias 技术（长计数器 32 位溢出模拟）

当硬件支持 64 位计数器（`PMCR.LP=1`）但用户请求的是 32 位事件时，内核使用 **bias 技术** 让 64 位计数器在低 32 位溢出时触发中断：
...
### 5.6 链式计数器（Chained Events）

对于 32 位计数器，可以将两个相邻计数器链接成一个 64 位计数器：
...
- 内核通过 armv8pmu_get_chain_idx() 分配连续偶奇对
```
...
![ARM64 PMU 采样中断路径时序图](image/arm64_pmu_sampling_irq_sequence.svg)
...
## 6. ARM64 PMU 驱动重要数据结构

### 6.1 `struct arm_pmu`（核心 PMU 抽象）

定义于 `include/linux/perf/arm_pmu.h`，是 ARM PMU 驱动的核心数据结构，封装了通用 `struct pmu` 并添加 ARM 特有的回调和状态：
...
![ARM64 PMU 关键数据结构关系图](image/arm64_pmu_key_data_structures.svg)
...
### 6.2 `struct pmu_hw_events`（每 CPU 硬件事件状态）
...
### 6.3 `struct hw_perf_event`（通用 perf 硬件事件状态）

嵌入在 `struct perf_event` 中（`event->hw`），保存每个事件的硬件层状态：
...
- `armpmu_event_init()`：填 `event->hw.config_base`，建立“事件语义”
- `armpmu_add()`：写 `event->hw.idx`，建立“槽位绑定”
- `armv8pmu_enable_event()`：把 `event->hw` 内容真正编程到 PMU 寄存器

---

## 7. ARM64 PMU 初始化流程与重要接口
...
![ARM64 PMU 初始化 / 注册路径时序图](image/arm64_pmu_init_registration_sequence.svg)
...
- DT 和 ACPI 入口不同，但最终都收敛到 `struct arm_pmu`
- `armv8pmu_probe_pmu()` 的能力探测发生在 `perf_pmu_register()` 之前
- `arm_pmu.c` 负责 generic glue，`arm_pmuv3.c` 负责 hardware personality
- 走到 `perf_pmu_register()` 时，PMU 只是完成“注册可见”，还没有任何 `perf_event` 上硬件计数器

#### 7.1.2 DT 路径 vs ACPI 路径差异对照图

<p align="center">
  <img src="image/arm64_pmu_dt_vs_acpi_comparison.svg" alt="ARM64 PMU DT 路径 vs ACPI 路径差异对照图" width="100%" />
</p>
...
- DT 侧是“先有 `platform_device` / 设备树节点，再解析 CPU 和 IRQ 关系”
- ACPI 侧是“先从 MADT/GICC 拿到 CPU 和 IRQ，再反向拼出 PMU 实例”
- DT 常保留驱动定义名，ACPI 常见 `name_%d` 形式的实例名
- 两条路径虽然枚举方式不同，但都会收敛到 `armv8_pmuv3_pmu_init()`、`armpmu_register()` 和 `perf_pmu_register()`

### 7.2 事件生命周期接口

#### 7.2.1 事件创建：`armpmu_event_init()`

```
用户调用 perf_event_open()
  │
  ▼
...
armpmu_event_init(event)                   ← arm_pmu.c
  ...
  └─ __hw_perf_event_init(event)
      ...
      │
      └─ validate_group(event)             ← 模拟分配验证组事件
```

#### 7.2.2 事件安装：`armpmu_add()`
...
armpmu_add(event, flags)                   ← arm_pmu.c
  ...
  └─ [PERF_EF_START] armpmu_start(event)
      ...
      └─ armpmu->enable(event)              ← armv8pmu_enable_event()
          ...
          └─ armv8pmu_enable_event_counter()← 写 PMCNTENSET
```

#### 7.2.3 事件移除：`armpmu_del()`

```
armpmu_del(event, flags)
  │
  ├─ armpmu_stop(event)
...
<p align="center">
  <img src="image/perf_event_to_pmu_lifecycle.svg" alt="perf_event 从创建到上 PMU 的生命周期图" width="100%" />
</p>
...
- `perf_event` 被创建，不等于已经占用 PMU 计数器
- `armpmu_event_init()` 完成事件映射，不等于已经选定具体 `idx`
- `armpmu_add()` 绑定了 `hw_events->events[idx]`，这才叫“上 PMU 槽位”
- `armpmu_start()` / `armv8pmu_enable_event()` 之后，硬件寄存器才真正开始计数

### 7.3 事件映射机制

事件映射将 perf 通用事件 ID 转换为硬件 PMU 事件编号：
...
## 8. ARM64 PMU 代码中的关键算法

### 8.1 计数器分配算法（`armv8pmu_get_event_idx`）

计数器分配策略遵循**专用优先、通用兜底**原则：
...
### 8.2 采样周期设置算法（`armpmu_event_set_period`）

此算法将"还需要多少事件才溢出"转换为硬件计数器的初始值：
...
### 8.3 计数器更新算法（`armpmu_event_update`）

无锁原子更新，处理硬件计数器的环绕（wrap-around）：
...
delta = (0x00001000 - 0xFFFFF000) & 0xFFFFFFFF
      = 0x00002000  ← 正确！跨越溢出点计算了 8192 个事件
```

### 8.4 事件组验证算法（`validate_group`）

验证一组事件能否同时放入有限的硬件计数器：
...
使用 fake_pmu 模拟计数器占用，验证所有事件能否同时就绪。
```

### 8.5 中断处理算法（`armv8pmu_handle_irq`）

```
armv8pmu_handle_irq(cpu_pmu):
┌─────────────────────────────────────────────────────────────────┐
│ 1. 读取并清除溢出标志                                              │
...
<p align="center">
  <img src="image/arm64_pmu_driver_framework.svg" alt="ARM64 PMU 驱动软件框架" width="100%" />
</p>
...
| 层级 | 代码位置 | 核心职责 |
|------|---------|---------|
| **用户空间** | `tools/perf/` | perf 命令行工具，通过系统调用与内核交互 |
| **Perf Event Core** | `kernel/events/core.c` | 事件生命周期管理、调度、ring buffer |
| **ARM PMU 通用框架** | `drivers/perf/arm_pmu.c` | 通用回调实现、事件映射、IRQ 分发、CPU 热插拔 |
| **PMUv3 驱动** | `drivers/perf/arm_pmuv3.c` | 具体寄存器操作、中断处理、计数器分配 |
...
PMI 中断         → armpmu_dispatch_irq()
                  → armv8pmu_handle_irq()
                      ...
                      → perf_event_overflow() → ring_buffer 写入
```

---

## 10. ARM64 PMUv3 驱动源码注释详解

以下是 `drivers/perf/arm_pmuv3.c` 中最核心的函数，附详细中文注释。
...
## 11. ARM64 Perf 全部命令详解与实战案例

> 本章基于 Linux 6.18.1 源码 `tools/perf/` 目录中注册的所有子命令，面向 ARM64 (AArch64) 平台给出具体用法和实战案例。
> 所有命令均可通过 `perf help <subcmd>` 查看完整手册。
...
> **编译依赖提示**：标有 ★ 的命令需要 `libtraceevent` 支持，标有 ◆ 的需要 `libelf` 支持。

---

### 11.2 性能剖析类命令

#### 11.2.1 `perf stat` — 精确性能计数器统计

**源文件**：`tools/perf/builtin-stat.c`
...
| 选项 | 说明 |
|------|------|
| `-e <event>` | 指定事件（支持逗号分隔多个事件） |
| `-a` | 系统全局（所有 CPU） |
| `-p <pid>` | 绑定到进程 |
| `-t <tid>` | 绑定到线程 |
| `-r <n>` | 重复运行 n 次，输出均值和标准差 |
| `-d` / `-dd` / `-ddd` | 详细级别（递增更多事件） |
| `-I <ms>` | 每隔 ms 毫秒打印一次中间结果 |
| `-G <cgroup>` | 按 cgroup 过滤 |
| `-o <file>` | 输出到文件 |
| `--per-core` | 按物理核汇总 |
| `--per-socket` | 按 CPU socket 汇总 |
| `--topdown` | 启用 Top-Down 微架构分析（需 PMU 支持） |
...
perf stat -e arm_spe_0/ts_enable=1/ -- ./my_program
```

---

#### 11.2.2 `perf record` — 采样记录

**源文件**：`tools/perf/builtin-record.c`
...
| `-a` | 系统全局采样 |
| `-p <pid>` | 采样指定进程 |
| `-g` | 记录调用栈（ARM64 默认使用 frame pointer） |
| `--call-graph <mode>` | 调用栈模式：`fp`（帧指针）、`dwarf`（DWARF CFI）、`lbr` |
| `-F <freq>` | 采样频率（Hz） |
| `-c <count>` | 每隔 count 个事件采样一次 |
| `-o <file>` | 指定输出文件（默认 `perf.data`） |
| `--filter <expr>` | BPF 过滤表达式 |
| `-d` | 记录地址信息（用于 `perf mem`） |
| `--switch-output` | 每 N 秒或 N 个采样分割输出文件 |
| `-m <pages>` | 环形缓冲区大小（页数） |
...
# 案例 11：监控已运行的进程
perf record -p $(pidof my_daemon) -g -- sleep 30
```

---

#### 11.2.3 `perf report` — 采样数据分析

**源文件**：`tools/perf/builtin-report.c`
...
| `--tui` | TUI 交互模式（默认） |
| `-n` | 显示采样数 |
| `-g <type>` | 调用图模式：`graph`（默认）、`flat`、`fractal`、`folded` |
| `--sort <keys>` | 排序键：`comm,dso,symbol,cpu,pid`... |
| `--percent-limit <n>` | 只显示占比 ≥ n% 的条目 |
| `--header` | 显示 perf.data 元信息 |
| `-F <fields>` | 自定义输出字段 |
| `--no-children` | 不累计子函数开销到父函数 |
| `--max-stack <n>` | 调用栈最大深度 |
| `-k <vmlinux>` | 指定 vmlinux 文件解析内核符号 |
...
perf report --no-children --stdio --percent-limit 0.5
```

---

#### 11.2.4 `perf annotate` — 源码/汇编级标注

**源文件**：`tools/perf/builtin-annotate.c`

**功能**：将采样数据标注到源码行或汇编指令级别，精确定位热点指令。

**基本语法**：
```bash
perf annotate [options] [symbol_name]
```
...
# 案例 6：标注指定 DSO 中的函数
perf annotate -d /usr/lib/libc.so.6 malloc
```

---

#### 11.2.5 `perf top` — 实时性能监视

**源文件**：`tools/perf/builtin-top.c`
...
#### 11.3.1 `perf trace` — 系统调用追踪 ★

**源文件**：`tools/perf/builtin-trace.c`
...
perf trace [options] -- <command> [<args>]
```
...
| 选项 | 说明 |
|------|------|
| `-e <syscall>` | 过滤特定系统调用（支持通配符） |
| `-p <pid>` | 追踪指定进程 |
| `-a` | 追踪全部 CPU |
| `-T` | 显示系统调用耗时 |
| `-s <field>` | 汇总模式排序字段 |
| `--summary` | 输出统计汇总 |
| `--duration <ms>` | 只显示耗时 ≥ ms 的系统调用 |
| `--max-events <n>` | 最多追踪 n 个事件 |
| `--filter-pids <pid>` | 排除指定 PID |
...
perf trace -e syscalls:sys_enter_write -e page-faults ./my_program
```

---

#### 11.3.2 `perf probe` — 动态探测点 ◆

**源文件**：`tools/perf/builtin-probe.c`
...
# 案例 10：查看可用的局部变量
perf probe --vars schedule_timeout
```

---

#### 11.3.3 `perf ftrace` — 内核 ftrace 前端

**源文件**：`tools/perf/builtin-ftrace.c`
...
perf ftrace [options] -- <command> [<args>]
```
...
# 案例 5：函数过滤 —— 只追踪调度相关函数
perf ftrace trace -T 'sched*' -a -- sleep 5
```

---

#### 11.3.4 `perf script` — 脚本化事件输出

**源文件**：`tools/perf/builtin-script.c`
...
#### 11.4.1 `perf c2c` — 缓存伪共享检测

**源文件**：`tools/perf/builtin-c2c.c`
...
# 案例 5：按缓存行地址分析
perf c2c report --stdio --coalesce tid,pid
```

---

#### 11.4.2 `perf mem` — 内存访问分析

**源文件**：`tools/perf/builtin-mem.c`
...
perf mem report --sort mem,node --stdio
```

---

#### 11.4.3 `perf lock` — 锁竞争分析 ★

**源文件**：`tools/perf/builtin-lock.c`
...
perf lock contention -a --lock-filter user -- sleep 10
```

---

#### 11.4.4 `perf sched` — 调度器分析 ★

**源文件**：`tools/perf/builtin-sched.c`
...
# 案例 8：查看特定 CPU 的调度情况
perf sched timehist --cpu 0,1
```

---

#### 11.4.5 `perf kmem` — 内核内存分配分析 ★

**源文件**：`tools/perf/builtin-kmem.c`
...
# 案例 6：按调用路径排序
perf kmem stat --caller --slab
```

---

#### 11.4.6 `perf kvm` — KVM 虚拟化分析

**源文件**：`tools/perf/builtin-kvm.c`
...
# 案例 4：追踪特定虚拟机
perf kvm --guest stat record -a -- sleep 10
```

---

#### 11.4.7 `perf kwork` — 内核 work 分析 ★

**源文件**：`tools/perf/builtin-kwork.c`
...
#### 11.4.8 `perf timechart` — SVG 时间图 ★

**源文件**：`tools/perf/builtin-timechart.c`
...
# 案例 6：调整时间图宽度
perf timechart -w 2000
```

---

#### 11.4.9 `perf bench` — 性能基准测试

**源文件**：`tools/perf/builtin-bench.c`
...
perf bench breakpoint enable
```

---

#### 11.4.10 `perf diff` — 对比分析

**源文件**：`tools/perf/builtin-diff.c`
...
perf diff config_a.data config_b.data
```
...
#### 11.5.1 `perf evlist` — 事件列表查看

**源文件**：`tools/perf/builtin-evlist.c`
...
# 案例 4：指定输入文件
perf evlist -i my_record.data
```

---

#### 11.5.2 `perf data` — 数据格式转换

**源文件**：`tools/perf/builtin-data.c`
...
# 案例 3：指定输入文件转换
perf data convert -i my_record.data --to-json output.json
```

---

#### 11.5.3 `perf inject` — 事件注入

**源文件**：`tools/perf/builtin-inject.c`
...
#### 11.5.4 `perf buildid-cache` — Build-ID 缓存管理

**源文件**：`tools/perf/builtin-buildid-cache.c`
...
# 案例 5：更新缓存
perf buildid-cache -u /path/to/updated_binary
```

---

#### 11.5.5 `perf buildid-list` — Build-ID 查询

**源文件**：`tools/perf/builtin-buildid-list.c`
...
#### 11.5.6 `perf archive` — 归档符号文件

**类型**：Shell 脚本（`perf-archive.sh`）
...
#### 11.6.1 `perf list` — 事件列表

**源文件**：`tools/perf/builtin-list.c`
...
![perf list 与 sysfs event_source 目录关联图](image/perf_list_to_sysfs_event_source.svg)
...
- `tools/perf/util/print-events.c` 负责把不同来源的事件整理成统一输出，并在 PMU 分支里遍历 `perf_pmu` 对象。
- `tools/perf/util/pmu.c` 才是真正读取 sysfs 的地方，它会扫描 `type`、`format/`、`events/`、`caps/`、`cpus` 等文件，把目录内容装配成用户态的 `struct perf_pmu`。
- 内核侧通过 `perf_pmu_register()` 创建 `event_source` 设备目录，所以 `perf list pmu` 看到的名字，本质上就是内核 PMU 注册信息的用户态投影。

也因此，看到诸如 `armv8_pmuv3/cache_misses/`、`armv8_pmuv3/cpu_cycles/`、`arm_spe_0/...` 这类名字时，可以直接反查 `/sys/bus/event_source/devices/<pmu>/events/` 和 `/sys/bus/event_source/devices/<pmu>/format/` 来理解它们的来源和编码含义。另一方面，`perf list hw` / `perf list sw` 的很多传统事件名，以及 `perf list tracepoint` 的事件，并不走这条 `event_source` sysfs 主干。

---

#### 11.6.2 `perf kallsyms` — 内核符号查找

**源文件**：`tools/perf/builtin-kallsyms.c`
...
#### 11.6.3 `perf config` — 配置管理

**源文件**：`tools/perf/builtin-config.c`
...
#### 11.6.4 `perf check` — 功能检测

**源文件**：`tools/perf/builtin-check.c`
...
# 案例 2：检查特定特性
perf check feature dwarf libelf bpf
```

---

#### 11.6.5 `perf daemon` — 守护进程模式

**源文件**：`tools/perf/builtin-daemon.c`
...
#### 11.6.6 `perf version` — 版本信息

**源文件**：`tools/perf/builtin-version.c`
...
#### 11.6.7 `perf iostat` — IO 统计

**类型**：Shell 脚本（`perf-iostat.sh`）
...
# 案例 2：指定设备
perf iostat -d sda -- sleep 10
```

---

### 11.7 ARM64 特有的 Perf 使用技巧

#### 11.7.1 使用原始 PMU 事件编号

ARM64 PMUv3 的事件可以通过原始编号访问，格式为 `rXX`（十六进制）或 `armv8_pmuv3/event=0xXX/`：
...
perf stat -e armv8_pmuv3/event=0x04/ ./my_program  # PMU 名称格式
```

#### 11.7.2 ARM64 SPE（Statistical Profiling Extension）

SPE 是 ARMv8.2 引入的硬件统计采样扩展，提供比 PMU 中断采样更精确的微架构信息：
...
# min_latency=N   只采样延迟 ≥ N 周期的操作
```

#### 11.7.3 ARM64 交叉分析工作流

在 ARM64 目标板采集数据，在 x86 主机上分析：
...
# 6. 生成火焰图
perf script -i perf.data | stackcollapse-perf.pl | flamegraph.pl > arm64_flame.svg
```

#### 11.7.4 ARM64 Cache 全方位追踪与分析

ARM64 上 Cache 性能分析是最常见的调优场景，涵盖 L1/L2/LLC/TLB 各级缓存。以下按「计数统计 → 采样定位 → 伪共享检测 → TLB 分析 → 高级追踪」分层讲解。
...
# rXX 形式，XX 为十六进制事件号
perf stat -e r04,r03,r15,r14,r01,r16,r17,r18 ./my_program
```

##### (2) Cache 计数统计（perf stat）
...
##### (3) Cache Miss 采样定位热点函数（perf record + report）

```bash
# 按 L1 数据缓存未命中采样 → 定位到具体函数
...
perf script | stackcollapse-perf.pl | flamegraph.pl \
  --title "LLC Miss Flame Graph" --colors mem > llc_miss_flame.svg
```

##### (4) Cache 伪共享检测（perf c2c）

```bash
# 记录 cache-to-cache 传输数据（多核伪共享核心工具）
...
##### (5) ARM64 SPE 精确 Cache 分析

SPE（Statistical Profiling Extension，ARMv8.2+）可以记录每次采样操作的**精确缓存层级命中信息**，比 PMU 中断采样更准确：
...
perf record -e arm_spe_0/pa_enable=1,load_filter=1,min_latency=50/ \
  -a -- sleep 10
perf script -F +phys_addr | head -20
```

##### (6) Cache Tracepoint 事件追踪

内核还提供了 tracepoint 级别的缓存相关事件：
...
# === 第七步：持续监控 ===
perf stat -e L1-dcache-load-misses,LLC-load-misses -I 1000 -a
```
...
perf script
perf probe --del probe:do_page_fault
```

#### 11.7.6 使用 Perf 定位软中断（SoftIRQ）过高问题

当 `/proc/softirqs` 或 `top` 中 `%si`（softirq）占比异常偏高时，说明某类软中断处理函数消耗了大量 CPU。以下是使用 perf 逐步定位的完整方法。
...
mpstat -P ALL 1
# 如果某个 CPU 的 %soft 远高于其他 → 可能是网络队列绑核不均
```

##### (2) perf top 实时定位软中断热点函数

```bash
# 实时查看内核态热点（软中断处理函数会出现在列表中）
perf top -a -U
# -U：隐藏用户态符号，只看内核
# 典型软中断热点函数示例：
...
##### (3) 使用 tracepoint 精确追踪软中断

内核提供了 `irq:softirq_entry` / `irq:softirq_exit` 两个 tracepoint，可以精确追踪每次软中断的类型和耗时：
...
##### (5) 量化软中断耗时（perf kwork）

```bash
# kwork 可以直接分析 softirq 的执行时间和延迟
perf kwork record --softirq -a -- sleep 10
...
# 时间线视图
perf kwork timehist --softirq
```

##### (6) 同时追踪硬中断 + 软中断（完整中断链路）
...
perf kwork latency --irq --softirq
```
...
# echo ff > /sys/class/net/eth0/queues/rx-0/rps_cpus  # 开启 RPS
```

---

#### 11.7.7 使用 Perf 定位 `top` 中 System（%sys）负载过高问题

当 `top` 命令显示 `%sy`（system/内核态 CPU 占用）异常偏高时，说明大量 CPU 时间花在了内核代码路径上。以下是使用 perf 系统化定位根因的完整方法。
...
# 生成内核态火焰图（直观定位）
perf script | stackcollapse-perf.pl | flamegraph.pl \
  --title "Kernel CPU Flame Graph" --colors kernel > kernel_flame.svg
```
...
# 追踪网络设备收发包
perf record -e net:netif_receive_skb,net:net_dev_xmit -a -- sleep 10
perf script | awk '{print $1}' | sort | uniq -c | sort -rn | head
```

##### (5) 第四步：量化内核态 vs 用户态时间占比

```bash
# perf stat 精确测量用户态/内核态 cycles 比例
...
#### 11.7.8 使用 Perf 定位应用程序（用户态）负载过高

当 `top` 中 `%us`（user）偏高时，说明 CPU 时间主要消耗在应用程序代码中。本节讲解如何用 perf 定位用户态热点，以及编译选项 `-g` 的作用和替代方案。

##### (1) 编译选项 `-g` 的作用与必要性

**核心问题：perf 需要应用程序用 `-g` 编译吗？**

| 场景 | 是否必须 `-g` | 说明 |
|------|:---:|------|
| `perf stat` 计数统计 | **不需要** | 只读硬件计数器，与二进制无关 |
| `perf top` / `perf report` 查看函数名 | **不需要** | 只要二进制没有被 `strip` 掉符号表（`.symtab`）即可 |
| `perf report` 查看调用栈（`-g`） | **看情况** | 见下方调用栈回溯方式 |
| `perf annotate` 标注到源码行 | **需要** | 需要 DWARF 调试信息（`-g` 或 `-g1`） |
...
**ARM64 的好消息**：ARM64 GCC 默认**不省略帧指针**（`-fno-omit-frame-pointer` 是默认行为），因此 **大多数 ARM64 应用无需额外编译选项即可获得完整调用栈**。
...
# 这样 perf report 仍能显示函数名和调用栈
```

##### (2) 不同二进制状态下 perf 的能力

```bash
# ===== 情况 A：完整调试信息（-g 编译，未 strip）=====
...
-p $(pidof my_program) -- sleep 10
# IPC > 2：计算密集型（正常）
# IPC < 0.5：可能被 cache miss / branch miss 拖慢
```
...
# 查看每个线程的 IPC
perf stat -e cycles,instructions --per-thread -p $(pidof my_mt_app) -- sleep 10
```

##### (5) 解释型语言 / JIT 程序（Java、Python、Node.js）

```bash
# Java 程序：使用 perf-map-agent 生成符号映射
...
#### 11.7.9 使用 Perf 生成火焰图（Flame Graph）完全指南

火焰图是性能分析中最直观的可视化工具，能一目了然地展示 CPU 时间在整个调用栈中的分布。
...
# flamegraph.pl          —— 将折叠格式转换为 SVG 火焰图
```
...
perf script | stackcollapse-perf.pl | flamegraph.pl \
  --title "Context Switch Flame Graph" --colors io > ctxsw.svg
```
...
#   2. 是否可以缓存解析结果避免重复解析
#   3. 是否可以改用二进制协议（protobuf/msgpack）替代 JSON
```
...
#   3. 使用 RCU 替代读写锁
#   4. 检查是否可以用 per-CPU / per-thread 数据消除竞争
```

**案例 D：Cache Miss 火焰图分析**
...
#### 11.7.10 `perf_event_paranoid` 权限控制

ARM64 系统中 perf 受 `/proc/sys/kernel/perf_event_paranoid` 控制：
...
## 12. QEMU 上 Perf 实践经典 Case

> **环境前提**：使用 QEMU 模拟 ARM64 环境（`-machine virt -cpu cortex-a72`），内核编译时打开
> `CONFIG_PERF_EVENTS=y`、`CONFIG_HW_PERF_EVENTS=y`、`CONFIG_DEBUG_INFO=y`、`CONFIG_KALLSYMS_ALL=y`。
> QEMU 默认提供 PMU 模拟（`-cpu host,pmu=on` 或 `-cpu cortex-a72`），大部分 cycles/instructions 事件可用。

### 12.1 Case 1：CPU 热点函数定位 — 死循环 / 计算密集
...
# 火焰图中 hot_function 是最宽的色块
```

**面试考点**：为什么需要 `-fno-omit-frame-pointer`？ → 因为 ARM64 编译器高优化级别可能省略帧指针（x29/FP），导致 `perf record -g`（默认 fp unwinding）无法正确回溯调用栈。替代方案是用 `--call-graph dwarf` 但开销更大。

---

### 12.2 Case 2：Cache Miss 分析 — 数组遍历顺序
...
```bash
aarch64-linux-gnu-gcc -O2 -g -fno-omit-frame-pointer \
  -o cache_miss cache_miss.c
```

#### 步骤三：用 perf stat 量化 Cache Miss
...
#### 步骤四：用 perf record 定位 Cache Miss 热点
...
# 生成 Cache Miss 火焰图
perf script | stackcollapse-perf.pl | flamegraph.pl \
  --title "L1D Cache Miss" --colors mem > cache_miss.svg
```

**关键结论**：列优先遍历每次访问跳跃 `COLS * sizeof(int)` = 16KB，远超 L1 Cache Line（64B），导致几乎每次访问都 miss。行优先遍历连续访问，一次 cache line fill 可覆盖 16 个 int。

---

### 12.3 Case 3：上下文切换分析 — 多线程锁竞争
...
```bash
aarch64-linux-gnu-gcc -O2 -g -fno-omit-frame-pointer \
  -o lock_contention lock_contention.c -lpthread
```

#### 步骤三：用 perf stat 观察上下文切换
...
#### 步骤四：用 perf lock 分析锁竞争

```bash
# 记录锁事件（需要内核支持 CONFIG_LOCK_STAT）
...
#### 步骤五：用 perf sched 分析调度延迟

```bash
perf sched record -- ./lock_contention
perf sched latency --sort max

#  预期输出：
#  Task               max(ms)    avg(ms)    count
#  lock_contention     15.2       0.8        500000+
#  → 线程频繁被调度出去等锁，最大延迟 15ms
```

#### 步骤六：优化方案 — 用原子操作替代锁

```c
/* 优化版本：用 __atomic 替代 mutex */
#include <stdatomic.h>
...
### 12.4 Case 4：Page Fault 分析 — 内存分配模式

**目标**：分析大量内存分配导致的 page fault 开销，用 perf 量化和定位
...
#### 步骤三：用 perf record 查看 fault 内核路径
...
#  可以看到内核为每一个 4KB 页单独分配
```

**关键结论**：
- `malloc` + 首次访问触发 65536 次 minor fault（256MB / 4KB）
- `MAP_POPULATE` 在 mmap 时预分配物理页，运行时几乎零 fault
- `MADV_HUGEPAGE` 让内核合并为 2MB 大页，fault 次数降至 128（256MB / 2MB）

---

### 12.5 Case 5：内核 tracepoint 追踪 — 调度延迟与 I/O

**目标**：用 perf 的 tracepoint 功能追踪内核调度和块 I/O 事件
...
#  0.060 write(fd=1, buf=0x..., count=4096) = 4096  (0.045 ms)
```

#### 步骤四：综合分析 — 使用 perf script 导出自定义格式
...
#   2000  block:block_rq_issue
```

**QEMU 注意事项**：
- QEMU 的 `-cpu` 参数需要加 `pmu=on` 才能使用硬件事件：`-cpu cortex-a72,pmu=on`
- 软件事件（`page-faults`, `context-switches`）和 tracepoint 无需 PMU，始终可用
- QEMU 模拟的 PMU 计数可能不完全精确，但相对比例有参考意义
- 多核场景需用 `-smp 4` 启动 QEMU

---

### 12.6 Case 6：False Sharing 检测 — 多核缓存行冲突

**目标**：演示多核访问同一 cache line 不同变量导致的性能下降
...
perf stat -e L1-dcache-load-misses,L1-dcache-store-misses \
  -- ./false_sharing
#  false sharing 版本的 miss 比无 sharing 版本高 10-50 倍
```

**修复原理**：ARM64 cache line 通常 64 字节。两个线程分别写 `counter_a` 和 `counter_b`，如果它们在同一 cache line，每次一个核写入都会让另一个核的 cache line 失效（MESI 协议），产生大量 coherence traffic。加 64 字节 padding 把它们分到不同 cache line 即可消除。

---

### 12.7 Case 7：内核模块性能分析 — 自定义驱动热点
...
## 13. Perf 高频面试题精选（附详细答案）

### 13.1 基础概念类

#### Q1：perf 的工作原理是什么？它是如何采样的？
...
- **硬件采样**：配置 PMU 计数器（如 cycles）每计数 N 次溢出，触发 PMI 中断，在中断处理中记录当前 PC 和调用栈
- **软件采样**：基于 hrtimer（高精度定时器）定期采样，不依赖硬件 PMU
- **ring buffer**：内核和用户态通过 mmap 共享环形缓冲区，零拷贝传递采样数据
- **采样 vs 计数**：`perf record`=采样（记录每次溢出时的上下文），`perf stat`=计数（只统计总数）

---

#### Q2：perf stat 和 perf record 的区别？各用在什么场景？

**答**：

| 对比项 | `perf stat` | `perf record` |
|--------|-------------|---------------|
...
#### Q3：什么是 PMU？ARM64 的 PMU 有什么特点？

**答**：

- **PMU**（Performance Monitoring Unit）是 CPU 内置的硬件性能计数器单元
- ARM64 PMU 遵循 **PMUv3 架构**（ARMv8 定义），通过系统寄存器访问
- 关键寄存器：
  - `PMCR_EL0`：PMU 控制寄存器（启用/复位/时钟分频）
  - `PMCNTENSET_EL0`：计数器使能集
  - `PMEVCNTR<n>_EL0`：事件计数器（通常 4-6 个通用 + 1 个 cycle 专用）
  - `PMEVTYPER<n>_EL0`：事件类型选择器
  - `PMINTENSET_EL1`：中断使能
  - `PMOVSSET_EL0`：溢出标志
- 特点：
  - 通用计数器数量有限（Cortex-A72 典型 6 个），所以 `perf stat` 多事件时需要**多路复用**（multiplexing）
  - 事件编号分**架构事件**（所有 ARMv8 核通用，如 0x08=instructions, 0x11=cycles）和**微架构事件**（厂商私有）
  - 支持 EL0/EL1/EL2 过滤（只统计用户态 or 内核态）
  - 高端核支持 SPE（Statistical Profiling Extension），在硬件层面记录 PC + 延迟 + 数据地址

---

#### Q4：perf 中的 IPC 是什么？如何判断 CPU 是否被充分利用？

**答**：

- **IPC** = Instructions Per Cycle（每周期执行指令数）= `instructions / cycles`
- 现代 ARM64 超标量核（如 Cortex-A76/X1/X2）理论峰值 IPC 可达 **4-8**
...
#### Q5：CPU 使用率高但 IPC 低，可能是什么原因？如何用 perf 排查？

**答**：CPU 使用率高只说明 CPU 没有 idle，但 IPC 低说明每个周期完成的有效工作少。常见原因：
...
perf report  # → 找到产生最多 miss 的函数
```

---

#### Q6：如何用 perf 排查一个进程的调度延迟？
...
# 4. NUMA 不亲和 → numactl 绑定 CPU 和内存
```

---

#### Q7：如何用 perf 分析一个内存泄漏或内存分配热点？

**答**：perf 不是专门的内存泄漏工具（推荐 kmemleak / valgrind），但可以分析内存分配行为：
...
perf report  # → 看调用栈确定谁在大量分配
```

---

#### Q8：perf record -g 的调用栈回溯有哪几种方式？各有什么优缺点？

**答**：

| 方式 | 命令参数 | 原理 | 优点 | 缺点 |
|------|---------|------|------|------|
| **Frame Pointer** | `-g` 或 `--call-graph fp` | 沿 FP 寄存器（x29）逐帧回溯 | 开销最小，速度最快 | 需要 `-fno-omit-frame-pointer` 编译；高优化可能被省略 |
| **DWARF** | `--call-graph dwarf` | 用 DWARF 调试信息解析栈帧 | 不需要 FP，支持所有优化级别 | 开销大（每次拷贝栈），数据量大 |
| **LBR** | `--call-graph lbr` | 用 CPU 的 Last Branch Record | 零开销获取调用链 | ARM64 **不支持** LBR（仅 x86）；深度有限（通常 8-32 层） |
...
# 大量 [unknown] 说明符号或调用栈有问题
```

---

#### Q9：什么是多路复用（multiplexing）？为什么 perf stat 有时显示 `<not counted>`？

**答**：

- ARM64 PMU 通常只有 **4-6 个通用计数器**
- 如果 `perf stat -e` 指定的事件数 **超过物理计数器数量**，perf 会自动**分时复用**
- 每个事件轮流占用计数器，最终值通过比例估算 `estimated_count = raw_count × (total_time / running_time)`
...
# - 权限不足（perf_event_paranoid 限制）
```

---

#### Q10：perf 和 ftrace 的区别？什么时候用 perf，什么时候用 ftrace？
...
选择决策树：
         ...
         ▼           ▼          ▼           ▼
      全局统计    函数热点    函数追踪    事件追踪
     perf stat  perf record  function_graph  trace_events
```
...
#### Q11：perf_event_open() 系统调用的核心参数有哪些？
...
| 字段 | 说明 |
|------|------|
| `type` | 事件类型：`PERF_TYPE_HARDWARE`, `PERF_TYPE_SOFTWARE`, `PERF_TYPE_TRACEPOINT`, `PERF_TYPE_RAW` |
| `config` | 事件编号：如 `PERF_COUNT_HW_CPU_CYCLES` |
| `sample_period` | 每 N 次事件触发一次采样（和 `sample_freq` 二选一） |
| `sample_freq` | 目标采样频率（Hz），内核自动调节 period |
| `sample_type` | 采样时记录哪些信息：`PERF_SAMPLE_IP`, `PERF_SAMPLE_CALLCHAIN`, `PERF_SAMPLE_TIME` 等 |
| `disabled` | 创建时是否禁用（需要 ioctl 启用） |
| `exclude_user` / `exclude_kernel` | EL0/EL1 过滤 |
| `mmap` | 是否追踪 mmap 事件（获取 DSO 映射信息） |
...
#### Q12：perf 的 ring buffer 是如何工作的？为什么是无锁的？
...
```
  ...
  - 溢出时数据丢弃（data_head 覆盖未读区域），体现在输出的 "lost N chunks"
```

面试追问：如果用户态读取太慢怎么办？ → 增大 ring buffer（`perf record -m 256M`），或提高采样间隔降低数据量。

---

#### Q13：perf 如何获取调用栈？内核态和用户态分别怎么处理？
...
符号解析流程（用户态 perf report）：
  ...
  4. 有 -g 调试信息时 → DWARF info → 源文件:行号
```

---

#### Q14：perf 事件的 exclude_kernel / exclude_user 是怎么实现的？

**答**：

ARM64 PMU 事件计数器的 `PMEVTYPER<n>_EL0` 寄存器中有两个过滤位：
...
perf stat -e cycles:uk -- ./my_app   # :uk = 两者都计（默认）
```

这是硬件级别的过滤，开销为零 — 这也是 PMU 相比纯软件计数器的重大优势。

---

### 13.4 高级场景类

#### Q15：如何用 perf 分析中断 / 软中断的性能影响？
...
# 如果某个中断号每秒增长 >10000，可能是中断风暴
```

---

#### Q16：如何在生产环境安全地使用 perf？
...
# tracepoint 高频事件：取决于事件频率
```

---

#### Q17：perf probe 是什么？如何动态追踪内核函数？

**答**：

`perf probe` 用于在**运行中的内核**中动态插入追踪点（基于 kprobe/uprobe），无需重新编译内核。
...
4. 断崖 — 父函数宽但子函数只占一部分 → 父函数本身也有计算开销
```
...
4. send_response 占 10% → I/O 发送部分开销可接受
```

---

#### Q19：perf 可以分析 Off-CPU 时间吗？如何做？
...
# On-CPU：函数执行慢 → 计算优化
# Off-CPU：函数等待长 → I/O / 锁 / 调度优化
```

Off-CPU 和 On-CPU 结合是性能分析的终极方法：如果 On-CPU 火焰图看不出问题（CPU 不热），那问题一定在 Off-CPU（等待 I/O、锁、网络）。

---

#### Q20：解释 perf 采样的 "skid" 问题，以及 ARM64 上如何缓解？
