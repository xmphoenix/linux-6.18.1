...
<details>
<summary><b>1. eBPF 内核支持选项与开关</b></summary>
...
<details>
<summary><b>2. eBPF 实现原理</b></summary>
...
<details>
<summary><b>3. eBPF 内核软件架构</b></summary>

- [软件架构总览图 (SVG)](#3-ebpf-内核软件架构)
- 3.1 [整体架构分层](#31-整体架构分层)
- 3.2 [核心数据结构关系](#32-核心数据结构关系) — 含数据结构关系图 (SVG)
- 3.3 [源码文件组织](#33-源码文件组织)
- 3.4 [程序类型与挂载点](#34-程序类型与挂载点)
- 3.5 [Map 类型体系](#35-map-类型体系)
- 3.6 [Link 抽象层](#36-link-抽象层)
- 3.7 [编译构建依赖](#37-编译构建依赖)
- 3.8 [子系统初始化流程](#38-子系统初始化流程) — 含初始化流程图 (SVG)
  - 3.8.1 [initcall 层级概览](#381-initcall-层级概览)
  - 3.8.2 [Level 0: pure_initcall — JIT 内存限额](#382-level-0-pure_initcall--jit-内存限额)
  - 3.8.3 [Level 1: core_initcall — 基础设施](#383-level-1-core_initcall--基础设施)
  - 3.8.4 [Level 4: subsys_initcall — BTF 与网络钩子](#384-level-4-subsys_initcall--btf-与网络钩子)
  - 3.8.5 [Level 5: fs_initcall — bpffs 与模块 BTF](#385-level-5-fs_initcall--bpffs-与模块-btf)
  - 3.8.6 [Level 7: late_initcall — 子系统主体](#386-level-7-late_initcall--子系统主体)
  - 3.8.7 [运行时按需初始化](#387-运行时按需初始化)
  - 3.8.8 [完整启动时序总结](#388-完整启动时序总结)
</details>
<details>
<summary><b>4. 如何生成一个 eBPF 程序</b></summary>
...
- 4.10 [交叉编译 ARM64 libbpf.so](#410-交叉编译-arm64-libbpfso)
  - 4.10.1 [源码结构与构建系统](#4101-源码结构与构建系统)
  - 4.10.2 [GCC 交叉编译工具链](#4102-gcc-交叉编译工具链)
  - 4.10.3 [LLVM/Clang 交叉编译](#4103-llvmclang-交叉编译)
  - 4.10.4 [交叉编译依赖库 (libelf / zlib)](#4104-交叉编译依赖库-libelf--zlib)
  - 4.10.5 [完整构建脚本](#4105-完整构建脚本)
  - 4.10.6 [产出文件与部署](#4106-产出文件与部署)
</details>
<details>
<summary><b>5. eBPF 完整指令集参考</b></summary>

- 5.1 [指令编码格式](#51-指令编码格式)
- 5.2 [ALU 算术逻辑指令](#52-alu-算术逻辑指令)
  - 5.2.1 [64 位 ALU 指令](#521-64-位-alu-指令-bpf_alu64-class0x07)
  - 5.2.2 [32 位 ALU 指令](#522-32-位-alu-指令-bpf_alu-class0x04)
  - 5.2.3 [字节序转换指令](#523-字节序转换指令)
- 5.3 [内存访问指令](#53-内存访问指令)
  - 5.3.1 [加载指令 (LDX)](#531-加载指令-ldx--寄存器间接加载)
  - 5.3.2 [符号扩展加载 (LDX MEMSX)](#532-符号扩展加载-ldx-memsx)
  - 5.3.3 [存储指令 (STX)](#533-存储指令-stx--寄存器到内存)
  - 5.3.4 [立即数存储 (ST)](#534-立即数存储-st--立即数到内存)
  - 5.3.5 [64 位立即数加载 (LD_IMM_DW)](#535-64-位立即数加载-ld_imm_dw)
  - 5.3.6 [传统包直接访问](#536-传统包直接访问-ld_abs--ld_ind)
- 5.4 [跳转指令](#54-跳转指令)
  - 5.4.1 [无条件跳转](#541-无条件跳转)
  - 5.4.2 [64 位条件跳转](#542-64-位条件跳转-bpf_jmp-class0x05)
  - 5.4.3 [32 位条件跳转](#543-32-位条件跳转-bpf_jmp32-class0x06)
  - 5.4.4 [条件伪跳转 (BPF_JCOND)](#544-条件伪跳转-bpf_jcond)
- 5.5 [函数调用与返回指令](#55-函数调用与返回指令)
- 5.6 [原子操作指令](#56-原子操作指令)
  - 5.6.1 [原子读-改-写](#561-原子读-改-写-支持-wdw-大小)
  - 5.6.2 [原子 Fetch 操作](#562-原子-fetch-操作-返回旧值到-src)
  - 5.6.3 [原子交换与比较交换](#563-原子交换与比较交换)
  - 5.6.4 [Load-Acquire 与 Store-Release](#564-load-acquire-与-store-release-内存序)
- 5.7 [内部 (非 UAPI) 指令](#57-内部-非-uapi-指令)
- 5.8 [指令集总览表](#58-指令集总览表)
- 5.9 [指令集版本演进](#59-指令集版本演进)
- 5.10 [Opcode 空间占用分析与扩展机制](#510-opcode-空间占用分析与扩展机制)
  - 5.10.1 [当前 opcode 空间使用情况](#5101-当前-opcode-空间使用情况)
  - 5.10.2 [各 Class 剩余空闲 opcode](#5102-各-class-剩余空闲-opcode)
  - 5.10.3 [已有的 opcode 扩展技巧](#5103-已有的-opcode-扩展技巧)
  - 5.10.4 [理论实际容量远超 256](#5104-理论实际容量远超-256)
  - 5.10.5 [未来超过 256 条的扩展策略](#5105-未来超过-256-条的扩展策略)
</details>
<details>
<summary><b>6. eBPF 底层挂载技术栈深度分析</b></summary>

- [技术栈全景图 (SVG)](#6-ebpf-底层挂载技术栈深度分析)
- 6.1 [技术栈总览](#61-技术栈总览)
- 6.2 [Kprobe / Kretprobe](#62-kprobe--kretprobe)
  - 6.2.1 [Kprobe 工作原理](#621-kprobe-工作原理)
  - 6.2.2 [BPF + Kprobe 挂载路径](#622-bpf--kprobe-挂载路径)
  - 6.2.3 [Kprobe Multi-Attach](#623-kprobe-multi-attach)
  - 6.2.4 [Kprobe Session](#624-kprobe-session)
  - 6.2.5 [完整示例](#625-完整示例)
- 6.3 [Tracepoint (静态跟踪点)](#63-tracepoint-静态跟踪点)
  - 6.3.1 [Tracepoint 工作原理](#631-tracepoint-工作原理)
  - 6.3.2 [BPF + Tracepoint 挂载路径](#632-bpf--tracepoint-挂载路径)
  - 6.3.3 [Raw Tracepoint](#633-raw-tracepoint)
  - 6.3.4 [完整示例](#634-完整示例)
- 6.4 [Ftrace / Fentry / Fexit / Fmod_ret](#64-ftrace--fentry--fexit--fmod_ret)
  - 6.4.1 [Ftrace 基础原理](#641-ftrace-基础原理)
  - 6.4.2 [BPF Trampoline 机制](#642-bpf-trampoline-机制)
  - 6.4.3 [Fentry / Fexit / Fmod_ret 对比](#643-fentry--fexit--fmod_ret-对比)
  - 6.4.4 [完整示例](#644-完整示例)
- 6.5 [Uprobe / Uretprobe](#65-uprobe--uretprobe)
  - 6.5.1 [Uprobe 工作原理](#651-uprobe-工作原理)
  - 6.5.2 [完整示例](#652-完整示例)
- 6.6 [Perf Event](#66-perf-event)
  - 6.6.1 [工作原理与 BPF 集成](#661-工作原理与-bpf-集成)
  - 6.6.2 [完整示例](#662-完整示例)
...
<details>
<summary><b>7. 网络子系统 eBPF 挂载点深度分析</b></summary>

- [数据路径全景图](#71-网络数据路径-bpf-挂载点全景)
- 7.1 [网络数据路径 BPF 挂载点全景](#71-网络数据路径-bpf-挂载点全景)
- 7.2 [XDP (eXpress Data Path)](#72-xdp-express-data-path)
  - 7.2.1 [XDP 上下文结构](#721-xdp-上下文结构)
  - 7.2.2 [XDP 动作码](#722-xdp-动作码)
  - 7.2.3 [内核执行路径](#723-内核执行路径)
  - 7.2.4 [Verifier / Ops 注册](#724-verifier--ops-注册)
  - 7.2.5 [XDP 专有 Helper 函数](#725-xdp-专有-helper-函数)
  - 7.2.6 [完整示例](#726-完整示例-xdp-丢弃指定端口流量)
- 7.3 [TC (Traffic Control) — SCHED_CLS / SCHED_ACT](#73-tc-traffic-control--sched_cls--sched_act)
  - 7.3.1 [TCX 架构 (6.x 新机制)](#731-tcx-架构-6x-新机制)
  - 7.3.2 [TC 上下文: __sk_buff](#732-tc-上下文-__sk_buff)
  - 7.3.3 [TC 动作码](#733-tc-动作码)
  - 7.3.4 [Verifier / Ops 注册](#734-verifier--ops-注册)
  - 7.3.5 [完整示例](#735-完整示例-tc-ingress-带宽限速)
- 7.4 [Socket Filter (SOCKET_FILTER)](#74-socket-filter-socket_filter)
- 7.5 [Cgroup BPF 网络程序](#75-cgroup-bpf-网络程序)
  - 7.5.1 [程序类型矩阵](#751-程序类型矩阵)
  - 7.5.2 [CGROUP_SKB 数据包过滤](#752-cgroup_skb-数据包过滤)
  - 7.5.3 [CGROUP_SOCK_ADDR 地址透明重写](#753-cgroup_sock_addr-地址透明重写)
  - 7.5.4 [完整示例](#754-完整示例-cgroup-出口流量统计)
- 7.6 [LWT (Lightweight Tunnel) BPF](#76-lwt-lightweight-tunnel-bpf)
  - 7.6.1 [三种挂载点](#761-三种挂载点)
  - 7.6.2 [通过路由表配置](#762-通过路由表配置)
  - 7.6.3 [LWT 特有返回值](#763-lwt-特有返回值)
- 7.7 [SOCK_OPS / SK_SKB / SK_MSG — Socket 级编程](#77-sock_ops--sk_skb--sk_msg--socket-级编程)
  - 7.7.1 [SOCK_OPS — TCP 连接生命周期事件](#771-sock_ops--tcp-连接生命周期事件)
  - 7.7.2 [SK_SKB / SK_MSG — Sockmap 数据重定向](#772-sk_skb--sk_msg--sockmap-数据重定向)
  - 7.7.3 [完整示例](#773-完整示例-socket-级透明代理)
...
<details>
<summary><b>8. eBPF 尾调用与 BPF-to-BPF 函数调用深度分析</b></summary>

- 8.1 [技术概览与对比](#81-技术概览与对比)
- 8.2 [尾调用 (Tail Call)](#82-尾调用-tail-call)
  - 8.2.1 [核心数据结构](#821-核心数据结构)
  - 8.2.2 [解释器执行路径](#822-解释器执行路径)
  - 8.2.3 [ARM64 JIT 实现](#823-arm64-jit-实现)
  - 8.2.4 [Verifier 校验](#824-verifier-校验)
  - 8.2.5 [完整示例: XDP 协议分层处理](#825-完整示例-xdp-协议分层处理)
- 8.3 [BPF-to-BPF 函数调用 (Subprog)](#83-bpf-to-bpf-函数调用-subprog)
  - 8.3.1 [子程序发现与注册](#831-子程序发现与注册)
  - 8.3.2 [Verifier 调用校验](#832-verifier-调用校验)
  - 8.3.3 [JIT 多函数编译 (jit_subprogs)](#833-jit-多函数编译-jit_subprogs)
  - 8.3.4 [ARM64 JIT 函数调用实现](#834-arm64-jit-函数调用实现)
  - 8.3.5 [全局函数与静态函数](#835-全局函数与静态函数)
  - 8.3.6 [完整示例: 模块化包解析](#836-完整示例-模块化包解析)
- 8.4 [BPF_PROG_TYPE_EXT — 运行时函数替换](#84-bpf_prog_type_ext--运行时函数替换)
  - 8.4.1 [工作原理](#841-工作原理)
  - 8.4.2 [完整示例: 动态替换全局函数](#842-完整示例-动态替换全局函数)
- 8.5 [尾调用与函数调用的组合使用](#85-尾调用与函数调用的组合使用)
- 8.6 [技术选型决策树](#86-技术选型决策树)
</details>
<details>
<summary><b>9. eBPF 性能分析实战：CPU / 内存 / I/O</b></summary>

- 9.1 [CPU 性能分析：函数调用追踪与热点代码检测](#91-cpu-性能分析函数调用追踪与热点代码检测)
  - 9.1.1 [分析目标](#911-分析目标)
  - 9.1.2 [内核挂载点源码分析](#912-内核挂载点源码分析)
  - 9.1.3 [完整示例：CPU 热点采样 + 函数耗时统计](#913-完整示例cpu-热点采样--函数耗时统计)
  - 9.1.4 [编译与运行](#914-编译与运行)
  - 9.1.5 [输出示例与解读](#915-输出示例与解读)
- 9.2 [内存分析：分配/释放追踪与泄漏检测](#92-内存分析分配释放追踪与泄漏检测)
  - 9.2.1 [分析目标](#921-分析目标)
  - 9.2.2 [内核挂载点源码分析](#922-内核挂载点源码分析)
  - 9.2.3 [完整示例：内存泄漏检测器](#923-完整示例内存泄漏检测器)
  - 9.2.4 [编译与运行](#924-编译与运行)
  - 9.2.5 [输出示例与解读](#925-输出示例与解读)
- 9.3 [I/O 性能分析：磁盘 I/O 与网络 I/O 监控](#93-io-性能分析磁盘-io-与网络-io-监控)
  - 9.3.1 [分析目标](#931-分析目标)
  - 9.3.2 [内核挂载点源码分析](#932-内核挂载点源码分析)
  - 9.3.3 [完整示例：磁盘 I/O + 网络 I/O 综合监控](#933-完整示例磁盘-io--网络-io-综合监控)
  - 9.3.4 [编译与运行](#934-编译与运行)
  - 9.3.5 [输出示例与解读](#935-输出示例与解读)
  - 9.3.6 [高级用法：per-进程 I/O 追踪](#936-高级用法per-进程-io-追踪)
- 9.4 [三大场景技术选型总结](#94-三大场景技术选型总结)
</details>
<details>
<summary><b>10. QEMU ARM64 eBPF 基础实验</b></summary>

- 10.1 [实验环境搭建](#101-实验环境搭建)
  - 10.1.1 [内核配置补全](#1011-内核配置补全)
  - 10.1.2 [QEMU 启动与 9P 共享](#1012-qemu-启动与-9p-共享)
  - 10.1.3 [交叉编译工具链](#1013-交叉编译工具链)
  - 10.1.4 [生成 vmlinux.h](#1014-生成-vmlinuxh)
...
- 10.8 [实验总结与故障排查](#108-实验总结与故障排查)
  - 10.8.1 [六组实验递进关系](#1081-六组实验递进关系)
  - 10.8.2 [常见故障排查表](#1082-常见故障排查表)
  - 10.8.3 [Guest 内常用调试命令](#1083-guest-内常用调试命令)
  - 10.8.4 [交叉编译静态 bpftool](#1084-交叉编译静态-bpftool-可选)
</details>
<details>
<summary><b>11. eBPF 面试高频问题与深度解答</b></summary>

- 11.1 [基础概念类](#111-基础概念类-必问) — Q1~Q5: eBPF 定义、执行流程、验证器、Map、程序类型
- 11.2 [JIT 编译与执行引擎](#112-jit-编译与执行引擎) — Q6~Q8: JIT 原理、ARM64 实现、Blinding
- 11.3 [挂载机制类](#113-挂载机制类) — Q9~Q12: kprobe/fentry/tracepoint 对比、Trampoline、XDP vs TC
- 11.4 [安全与限制类](#114-安全与限制类) — Q13~Q15: 安全限制、写权限、非特权使用
- 11.5 [性能与实践类](#115-性能与实践类) — Q16~Q20: 开销量化、调试方法、CO-RE、尾调用、Ringbuf
- 11.6 [网络子系统类](#116-网络子系统类) — Q21~Q23: XDP 模式、防火墙、CNI
- 11.7 [内存与 Map 操作类](#117-内存与-map-操作类) — Q24~Q26: 并发安全、调用栈采集、内存分配
- 11.8 [架构与系统设计类](#118-架构与系统设计类) — Q27~Q29: 可观测平台、模块替代、ARM64 嵌入式
- 11.9 [高级与前沿类](#119-高级与前沿类) — Q30~Q33: kfunc、BTF、有界循环、用户空间访问
- 11.10 [场景设计类](#1110-场景设计类) — Q34~Q37: 内存泄漏、TCP 监控、负载均衡、安全审计
- 11.11 [编码与调试实操类](#1111-编码与调试实操类) — Q38~Q40: 判空、CORE_READ、浮点
- 11.12 [面试快问快答](#1112-面试快问快答-10-题) — Q41~Q50: 十问十答速查表
</details>
